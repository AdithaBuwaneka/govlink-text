...
interface ChatMessage {
  id: string;
  // FIX: Added 'system' to the sender type to match the parent component.
...
const ChatSession: React.FC<ChatSessionProps> = ({
  ...
  return (
    <div className="glass-morphism rounded-2xl border border-border/50 shadow-glow h-[600px] flex flex-col animate-fade-in-up">
      {/* Chat Header */}
      <div className="p-4 border-b border-border/30">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            {/* Avatar */}
            <div className="w-10 h-10 bg-gradient-to-r from-[#FFC72C] to-[#FF5722] rounded-full flex items-center justify-center text-white font-semibold">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 bg-card/30 border border-border/50 rounded-lg text-xs font-medium text-foreground hover:bg-card/50 hover:border-[#FFC72C] transition-all duration-300"
            >
              {t.transferChat}
            </button>
            <button
              onClick={onEndChat}
              className="px-3 py-1.5 bg-gradient-to-r from-[#FF5722] to-[#8D153A] text-white rounded-lg text-xs font-medium hover:from-[#8D153A] hover:to-[#FF5722] transition-all duration-300"
            >
              {t.endChat}
            </button>
          </div>
      ...
      >
        {/* Drag overlay */}
        {dragOver && (
          <div className="absolute inset-4 bg-[#FFC72C]/10 border-2 border-dashed border-[#FFC72C] rounded-xl flex items-center justify-center">
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="max-w-xs mr-12">
              <div className="p-3 bg-card/50 border border-border/30 rounded-2xl">
      ...
      <div className="p-4 border-t border-border/30">
        <div className="flex gap-3">
          ...
          <button
            ...
            className="p-2 bg-card/30 border border-border/50 rounded-lg text-muted-foreground hover:text-foreground hover:bg-card/50 hover:border-[#FFC72C] transition-all duration-300 flex-shrink-0"
            title={t.attachFile}
          >
          ...
          <div className="flex-1 relative">
            <textarea
              ...
              className="w-full bg-card/30 border border-border/50 rounded-lg px-4 py-3 pr-12 text-foreground placeholder-muted-foreground focus:outline-none focus:border-[#FFC72C] transition-all duration-300 resize-none overflow-hidden min-h-[48px] max-h-[120px]"
              rows={1}
            />
            ...
            <button
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-gradient-to-r from-[#FFC72C] to-[#FF5722] text-white rounded-lg hover:from-[#FF5722] hover:to-[#8D153A] transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:from-[#FFC72C] disabled:hover:to-[#FF5722]"
              title={t.sendMessage}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
