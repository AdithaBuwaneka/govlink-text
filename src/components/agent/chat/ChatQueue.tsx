    ...
    <div className="glass-morphism p-6 rounded-2xl border border-border/50 shadow-glow animate-fade-in-up">
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-xl font-semibold text-foreground">{t.queueTitle}</h3>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-[#008060] rounded-full animate-pulse"></div>
            <span className="text-sm text-muted-foreground">{chatQueue.length}</span>
          </div>
        </div>
        <div className="w-full h-px bg-gradient-to-r from-[#FFC72C] to-[#FF5722] rounded-full"></div>
      </div>
      ...
      <div className="space-y-4 max-h-[500px] overflow-y-auto">
        {chatQueue.length > 0 ? (
          chatQueue.map((request, index) => (
            <div 
              key={request.id} 
              className="group glass-morphism p-4 rounded-xl border border-border/30 hover:border-[#FFC72C] transition-all duration-300 hover:scale-[1.02] animate-fade-in-up"
            ...
            >
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  {/* Avatar */}
                  <div className="w-10 h-10 bg-gradient-to-r from-[#FFC72C] to-[#FF5722] rounded-full flex items-center justify-center text-white font-semibold text-sm">
                ...
                {request.priority === 'urgent' && (
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold border ${getPriorityColor(request.priority)} animate-pulse`}>
                    {t.urgent}
                  </span>
                )}
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 px-3 py-2 bg-gradient-to-r from-[#008060] to-[#FFC72C] text-white rounded-lg text-sm font-medium hover:from-[#FFC72C] hover:to-[#FF5722] transition-all duration-300 hover:scale-105 shadow-sm"
                >
                  {t.accept}
                </button>
                <button
              ...
              {/* Hover Effect */}
              <div className="absolute inset-0 bg-gradient-to-br from-[#FFC72C]/5 via-transparent to-[#FF5722]/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-xl pointer-events-none"></div>
            </div>
          ))
        ) : (
          /* Empty State */
