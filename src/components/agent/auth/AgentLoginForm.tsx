...
const AgentLoginForm: React.FC<AgentLoginFormProps> = ({ language = 'en' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.employeeId.toUpperCase() === 'DEMO1234') {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {errors.general && (
        <div className="bg-[#FF5722]/10 border border-[#FF5722]/20 rounded-xl p-4 animate-fade-in-up">
        ...
        <div className="relative">
          ...
          >
            ...
            <select
              ...
              required
              className="w-full bg-transparent text-foreground p-4 sm:p-5 pl-12 sm:pl-14 rounded-xl focus:outline-none transition-all duration-300 text-base leading-relaxed appearance-none cursor-pointer"
            >
      ...
      <div className="flex items-center justify-between animate-fade-in-up">
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm text-muted-foreground">{t.keepMeSignedIn}</span>
        </label>
      </div>
      ...
      <button
        ...
        disabled={isLoading}
        className={`
          w-full relative overflow-hidden rounded-xl font-semibold text-base sm:text-lg py-4 sm:py-5 transition-all duration-300 shadow-glow
          ${isLoading 
            ? 'bg-muted text-muted-foreground cursor-not-allowed' 
            : 'bg-gradient-to-r from-[#FFC72C] to-[#FF5722] hover:from-[#FF5722] hover:to-[#8D153A] text-white hover:scale-[1.02] hover:shadow-2xl'
          }
          animate-fade-in-up
        `}
      >
        ...
        {/* Button Hover Effect - SAME as landing page */}
      ...
      <div className="bg-card/30 border border-border/50 rounded-xl p-4 animate-fade-in-up">
        <div className="flex items-start gap-3">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-[#FFC72C] mt-0.5 flex-shrink-0">
            <circle cx="12" cy="12" r="10"/>
            <path d="M12 16v-4"/>
            <path d="M12 8h.01"/>
          </svg>
          <div>
            <p className="text-sm font-medium text-foreground mb-1">{t.demoAccess}</p>
            <p className="text-xs text-muted-foreground">
              Employee ID: <span className="font-mono bg-muted px-1 py-0.5 rounded">DEMO1234</span>
              <br />
              Password: <span className="font-mono bg-muted px-1 py-0.5 rounded">demo123456</span>
            </p>
          </div>
        </div>
      </div>
