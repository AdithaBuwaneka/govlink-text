...
const ChatSidebar: React.FC<ChatSidebarProps> = ({
  ...
  if (!activeChat) {
    return (
      <div className="glass-morphism p-6 rounded-2xl border border-border/50 shadow-glow h-[600px] flex items-center justify-center animate-fade-in-up">
      ...
      <div className="p-4 border-b border-border/30">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(activeChat.status)}`}>
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {activeTab === 'info' && (
          <div className="space-y-4">
            {/* Basic Info */}
            <div>
              <h4 className="text-sm font-semibold text-foreground mb-3">{t.citizenInfo}</h4>
            ...
            <div className="pt-4 border-t border-border/30">
              <h4 className="text-sm font-semibold text-foreground mb-3">{t.chatHistory}</h4>
        ...
        {activeTab === 'actions' && (
          <div className="space-y-2">
            <h4 className="text-sm font-semibold text-foreground mb-3">{t.quickActions}</h4>
            {mockActions.map((action) => (
              <button
                key={action.id}
                onClick={() => console.log(`Action: ${action.id}`)}
                className="w-full p-3 bg-card/20 hover:bg-card/40 border border-border/30 hover:border-[#FFC72C] rounded-lg text-left transition-all duration-300 hover:scale-[1.02] group"
              >
